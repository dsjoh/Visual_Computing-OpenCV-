...
def make_panorama(img_left, img_right, reverse=False):  # 파노라마 이미지를 만드는 함수
    ...
    # ORB로 특징점 추출
    orb = cv2.ORB_create()
    keypoints_left, descriptors_left = orb.detectAndCompute(img_gray_left, None)
    keypoints_right, descriptors_right = orb.detectAndCompute(img_gray_right, None)
    ...
    # 상위 N개의 좋은 매치 선택 
    good_correspondences = matches[:100]
    ...
    H, _ = cv2.findHomography(dst_pts, src_pts, cv2.RANSAC, 20.0)  # 호모그래피 계산에는 RANSAC을 사용 
